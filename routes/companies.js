...
router.get("/", async function (req, res, next) {
  try {
    ...
    const companiesSearch = { name, minEmployees, maxEmployees };
    const validator = jsonschema.validate(companiesSearch, companySearchSchema);
    if (!validator.valid) {
      const errs = validator.errors.map((e) => e.stack);
      throw new BadRequestError(errs, 404);
    }
    const companies = await Company.findAll(companiesSearch);
